...
from embedder.renemb_finetune_rowclass.model import (
    RenembFinetuneRowClassification,
)
from embedder.renemb_finetune_rowclass.data_module import LineClassDataModule
...
class StrugritteEvaluator(Evaluator):
    def __init__(
        ...
        **kwargs,
        ...
        self.renemb_features_file = renemb_features_file
        ...
        self.renemb = RenembFinetuneRowClassification(
    ...
    def renemb_features(self, dl, *args, **kwargs):
        ...
        for batch in batches:
            ...
            for file_batch_idx, line_prediction in enumerate(predicted):
                if self.renemb.empty_index in target[file_batch_idx]:
                    num_lines =  target[file_batch_idx].tolist().index(self.renemb.empty_index)
                else:
                    num_lines = self.renemb.max_rows
    ...
    def evaluate(self, *args, **kwargs):
        ...
        if not os.path.exists(self.renemb_features_file):
            dl = self.dm.full_dataloader()
            renemb_df = self.renemb_features(dl)
            renemb_df = renemb_df.join(
                pd.DataFrame(
                    renemb_df["row_embeddings"].tolist(),
                    index=renemb_df.index,
                    columns=[f"row_embedding_{i}" for i in range(768)],
                )
            )
            del renemb_df["row_embeddings"]
            renemb_df.to_csv(self.renemb_features_file, index=False)
        else:
            renemb_df = pd.read_csv(self.renemb_features_file)

        if not self.use_row_cls:
            renemb_df = renemb_df[
                [c for c in renemb_df.columns if not c.startswith("row_embedding")]
            ]

        if not self.use_lineprobs:
            renemb_df = renemb_df[
                [c for c in renemb_df.columns if not c.endswith("_logits")]
            ]

        features_df = pd.merge(strudel_df, renemb_df, on=["filename", "line_number"])
    ...
    for r, l in [(False, True), (False, False)]:
        for dataset in ["mendeley", "troy"] + train_datasets:
            ...
            evaluator = StrugritteEvaluator(
                ...
                cuda_device=0,
                weights_path=f"weights/finetune_lineclass/renemb_lineclass{dataset}.pth",
                ...
                renemb_features_file=f"weights/finetune_lineclass/renembfeatures_{dataset}.csv",
            )
            evaluator.evaluate()
