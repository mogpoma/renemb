...
class RenembEvaluator(Evaluator):
    def __init__(
        self,
        config_path="configs/rowclass.jsonnet",
        weights_path="",
        global_max=True,
        cuda_device=-1,
        batch_size=16,
...
if __name__ == "__main__":
    ...
    for dataset in train_datasets + ["mendeley","troy"]:
    # for dataset in ["mendeley", "troy"]:
        ...
        for rep in range(n_reps):
            ...
            evaluator = RenembEvaluator(
                data_dir=f"data/row_classification/",
                sys_name=f"renemb{rep}",
                experiment_dir="results/finetune_rowclass/",
                rep_num = rep,
                ...
                n_workers=100,
                cuda_device=0,
                weights_path=f"weights/rowclass/renemb_lineclass{dataset}.pth",
            )
            evaluator.evaluate()
    ...
    pd.DataFrame.from_dict(results, orient="index", columns=[f"renemb_{rep}" for rep in range(n_reps)]).to_csv("results/renemb_results.csv", index_label="measure")
