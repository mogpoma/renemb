...
class RenembPretrainingVAE(RenembBase):
    """
    Renemb Model with a head on top of the row embedders for the file encoding pretraining:
    ...
    def __init__(self,
        ...
        super(RenembPretrainingVAE, self).__init__(*args, **kwargs)
    ...
    def forward(self, input_tokens, *args, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        y = super(RenembPretrainingVAE, self).forward(input_tokens, *args, **kwargs)
        ...
        # output : [batch_size, n_rows, max_len, d_model],
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.parameters()), lr=self.optimizer_lr, fused=True)
        return optimizer
    ...
    def training_step(self, batch, batch_idx):
        # output, loss = self.forward_loss(batch)
